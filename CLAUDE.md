...
As of 2026-08-22, every page under `hp/` (`index.html`, `case-study/index.html`, `use-cases/index.html`, `youtube/index.html`, `ai-research/index.html`, `p66-example/index.html` as of 2026-08-23, `30-60-90/index.html` as of 2026-08-27, and `object-detect-summary/index.html` plus `object-detect-level-1/index.html` through `object-detect-level-10/index.html` (ten levels, expanded from six on 2026-09-21) as of 2026-09-20 — eighteen pages total) shares one persistent left sidebar nav (`.hp-shell` / `.hp-sidenav` / `.hp-main` etc. — the CSS block is duplicated verbatim into each page's `<style>`, since there's no build step to share it from one file). This sidebar is deliberately its own design layer, sitting *outside* the light-editorial system every `/hp` page's content area now uses (see above): a dark (`#14161b`) fixed-left sidebar (220px, tightened from 232px on 2026-09-21 — see below) with the HP brand blue (`#0096D6`, defined as `--hp-blue` in every `/hp` page's own `:root` as of 2026-08-26, and always referenced as `var(--hp-blue, #0096D6)` with the hardcoded fallback kept for safety) as the active-link/accent color. This is the only place `--hp-blue` is used — the content area of every `/hp` page uses `--blue` (#1967b1) like `index.html`/`projects.html` instead; `hp/youtube/index.html` briefly leaked `--hp-blue` into its content-area "now playing"/"Featured" styling and was fixed back to `--blue` on 2026-08-26 to keep this rule true everywhere. The main link list, in order, all flat/same-level (six total): "Home - Job Fit" (`/hp` — relabeled from plain "Home" on 2026-08-23 so the sidebar itself hints at what that page is), ZGX Nano Case Study (`/hp/case-study`), Use Cases (`/hp/use-cases`), Youtube (`/hp/youtube`), AI Research (`/hp/ai-research`), P66 Example (`/hp/p66-example`). Below that flat list sit two more visually separated groups, each a `.hp-nav-section` div (top border + `.hp-nav-section-label` uppercase label, both dark-sidebar-only styles, no `--blue`/`--hp-blue` conflict since they're just gray) wrapping its own nav. "Object Detect" (`.hp-nav-section-label` "Object Detect", added 2026-09-20) comes first: "Summary" as a normal `.hp-nav-link` and Levels 1-10 as a compact 5-column numbered grid (see below), not eleven stacked links. Started at six levels on 2026-09-20, expanded to ten on 2026-09-21 — Levels 7-10 are pure `.panel-empty`/`.empty-tag` placeholders (same "Content Coming Soon" pattern the whole section started from) with a neutral `.level-status.pending` "TBD" pill, since Avi hasn't defined what those levels are yet; don't invent content for them. "Post Interview" (added 2026-08-27, one link: "30/60/90 Plan", `/hp/30-60-90`) comes second, now at the bottom of the whole sidebar. **This order was deliberately flipped on 2026-09-22** — Post Interview was added first chronologically and originally sat above Object Detect, but Avi asked to put Object Detect above Post Interview with Post Interview at the very bottom; don't revert to chronological/add order without being asked again. **That 2026-09-22 fix only reached the 8 hand-maintained `/hp` pages — the 10 `object-detect-level-*` pages (generated from `build_levels.py`) still had the two `.hp-nav-section` blocks in the old Post-Interview-first order, since the generator script's own template wasn't updated at the time.** Caught and fixed on 2026-09-22 (later the same day) by swapping the two blocks in `build_levels.py`'s page template and regenerating all 10 levels — verified by grepping `hp-nav-section-label` order across all 18 `/hp` pages afterward. Lesson: a nav-order (or any shared-chrome) change to `/hp` pages must be applied to `build_levels.py`'s template too, not just the 8 hand-edited files, or the next regeneration silently reverts it. Each group is its own `.hp-nav-section` block, not nested inside one another. If Avi asks for more pages within an existing group (post-interview or object-detect), add them as more links inside that group's own `.hp-nav-section` block, not as new top-level flat items or a new group; only start a new `.hp-nav-section` when he explicitly asks for a new, differently-named group — and ask where it should sit relative to the existing two rather than assuming it goes at the bottom. Copy the `.hp-nav-section`/`.hp-nav-section-label` CSS (added right after `.hp-nav-link.active` in every page's shared nav CSS block) and the section HTML (right before `</aside>`, Object Detect block first then Post Interview block) to all eighteen pages together, same as any other nav change. Deliberately **no** link back to the main site (`/`) — Avi removed it on 2026-08-22 so the sidebar doesn't distract a reader who's evaluating this HP-specific material. Don't re-add one unless he asks.
...
**Sidebar v5 (2026-09-25, per Avi) — current order, overrides v4's order below:** only two main items, **Home** and **Projects**, as peers (no divider between them anymore; `.side-group` is just `margin-top: 2px`). AI Arcade moved from the main list into the Projects list, so the indented projects are now: Robotic Surgery ↗, Object Detect (+ level grid), **AI Arcade ↗** (still `/games` in a new tab), Clutch Sports Data. Generator: `sidebar_v5.py` in the session scratchpad (not tracked). Rebuild a script like it rather than editing 13 pages by hand.

**Sidebar v4 — current structure (2026-09-25, per Avi; v3's visual style below still applies unless noted).** Order: brand, then Home, AI Arcade (↗, new tab), then a separate `.side-group` "Projects" section (divider above it). "Projects" (`.side-group-head`, links to `/projects.html`) is a main-size item. Indented under it (`.side-sub`, left guide line) are the individual projects, in this order: **Robotic Surgery** (external, new tab, ↗, to `avikravi.github.io/surgical-robotics-tissue-sim/dataset_viewer.html`, since there's no main-site page for it yet), **Object Detect** (`/object-detect`, with the 1-10 level-button grid directly under it; there's no separate "Summary" link anymore, since Object Detect itself is the summary), **Clutch Sports Data** (`/projects.html#clutch-sports-data`, an `id` added to that card on projects.html). The brand no longer has the headshot (Avi: two photos on the homepage looked strange, and the photo belongs on the homepage), and the role line reads "AI/ML Engineer". Active states: Home/Projects get the white selected-tab look; on Object Detect pages the Object Detect sub-link gets it on the summary, the level button goes black on level pages, and "Projects" turns black (`.current`). To add a project: add a `.side-sub-link` inside `.side-sub` on all 13 pages via the generator script. Mobile: rows are Home / AI Arcade / Projects, then the project links, then the level row (only shown on Object Detect pages, via `.site-side.in-od`).

**Sidebar v3 — current design (2026-09-25, per Avi: v2 was "boring" and "not obvious what to click," wanted clean, modern, "not AI-looking," consistent with the site).** 232px wide, `#fafafa` background with a `--line` right border (so it reads as its own panel against the white page). Top: `.side-brand` = round headshot (`/headshot.jpg`) + "Avi Ravishankar" + "Data Scientist · PM", linking home, with a divider under it (replaces the old "AR" wordmark). Main links are sentence-case 14px `--ink-soft` text, each with a simple 18px line-icon SVG (`.side-icon`: house, 2x2 grid, gamepad, viewfinder); hover = `#efefef` fill; active = white "selected tab" (`#fff` + 1px `--line` ring + faint shadow, bold, black icon). AI Arcade has a small ↗ `.side-ext` arrow since it opens in a new tab. Object Detect's sub-items sit in an indented `.side-sub` (left guide line): a 13px "Summary" `.side-sub-link`, a tiny "LEVELS" `.side-sub-label`, then the 1-10 grid as bordered white 28px buttons (hover = black border, active = solid black). On OD pages the "Object Detect" main link gets `.active` too. Deliberately no gradients, glows, emoji, or colored accents; black/white/gray only, plus `--blue` for keyboard focus rings. Mobile (<760px): stacks as a top header: brand row, then the four links in one row (icons and ↗ hidden so they fit), then Summary + a 10-column level row. Same block on all 13 sidebar pages; regenerate it with one script, not by hand. The v2 notes below are history.
...
**`object-detect/` (public copy of `/hp/object-detect-*`).** Created 2026-09-25 by copying all 11 `/hp` Object Detect folders (with their images/CSVs) and converting them with a scratchpad script: swapped the dark `/hp` sidebar CSS/HTML/toggle script for the main-site sidebar above; removed `content_group: 'HP Portfolio'` from `gtag('config')` (plain snippet, like `index.html`); removed the `noindex, nofollow` robots meta (these are meant to be public); replaced the "Working draft, not for external distribution yet." footer with "&copy; 2026 Avi Ravishankar"; rewrote internal links to `/object-detect/...`; and replaced the Summary dek's interview framing ("Take-home deliverable for a ... interview (round 4)") with a neutral one-line project description. Everything else, including Levels 9-10's HP ZGX Nano/Fury scaling writeups, is identical content. `/hp` was not touched. **These are now two independent copies**: an edit to a `/hp/object-detect-*` page (or a `build_levels.py` regeneration) does NOT flow to `/object-detect`, and vice versa. When Avi changes project content, ask whether it should go in both, and apply it to both if so. Styling differs only in chrome (sidebar, footer, dek); the Summary page keeps its Prettier formatting, like its `/hp` original. **The public Summary (`object-detect/index.html`) was further cleaned up on 2026-09-25, per Avi, and now intentionally differs from `/hp/object-detect-summary`:** no intro YouTube walkthrough video (removed along with its CSS and `embed_viewed` event; don't re-add it); no intro paragraph repeating the dek; the Level Roadmap table has no Status column (every level was "Done"); "Overall Narrative Arc" renamed "How the Levels Build"; sections reordered to Level Roadmap → How the Levels Build → Environment; and leftover unused CSS (slides-embed, panel-empty, level-status variants, roadmap-note) removed.
