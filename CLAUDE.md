...
hp/                      All of Avi's HP application material, as of 2026-08-22. Every page here
  ...
  object-detect-summary/index.html  URL: /hp/object-detect-summary. Added 2026-09-20. First page
                         under a new "Object Detect" grouped sidebar section (see "The /hp
                         section" below) for a separate object-detection project Avi is working
                         on. Currently a `.panel-empty`/`.empty-tag` placeholder ("Slides Coming
                         Soon") — Avi described it as "a collection of slides," so the plan is a
                         presentation embed once he sends the actual deck (Google Slides link,
                         PDF export, or exported images all plausible; format not yet confirmed).
                         A `.slides-embed` CSS rule (same iframe-container pattern as
                         `.sim-embed`/`.app-embed` on other /hp pages) is already in the
                         stylesheet, unused until the real embed is wired up — reuse it rather
                         than inventing a new container when the content arrives.
  object-detect-level-1/index.html  URL: /hp/object-detect-level-1. Added 2026-09-20 alongside
                         object-detect-summary, as the second link in the same "Object Detect"
                         section. Pure placeholder — Avi said he'd provide what goes here later;
                         don't invent content for it. Same `.panel-empty` placeholder pattern.
