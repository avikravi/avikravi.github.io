...
| `/hp/youtube` | `hp/youtube/index.html` | Youtube playlist of Avi's AI/ML project videos — a vertical "All Videos" list on the left (click an item to switch) and a big "now playing" player filling the rest of the row on the right. The video list (`hp/youtube/videos.json`) is manually maintained, not live-synced — see `CLAUDE.md` for why. As of 2026-08-25, the first list item is a hardcoded, non-YouTube video — a Loom embed of "P66 Vessel Integrity Intelligence," tagged "Featured" and active by default — inside the same list/player layout rather than a separate boxed section (an earlier boxed version was tried and rejected). See `CLAUDE.md` for how to change it. |
