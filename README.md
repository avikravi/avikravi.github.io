...
| `games/` | Games hub (2026-09-25): an arcade-style "Select Your Game" screen (`index.html` + `style.css` + `script.js`) with the main-site sidebar. First game: Thermostat Panic (`games/thermostat/`), keep the room at 68-72°F for 60 seconds. See `CLAUDE.md` for how to add a game. |
...
As of 2026-08-21, `index.html` and `projects.html` use a light-editorial design system (matching `hp/case-study/index.html` — see `CLAUDE.md` for the token reference), while `tracker.html` remains on the original neon dark design (its interactive UI would need separate rework to restyle safely). As of 2026-09-25, `index.html` and `projects.html` share a byte-for-byte identical, always-visible left sidebar nav (sourced from `index.html`; only the `active` link differs) with Home, AI Projects, and Games (`/games`). Below those, an "Object Detect" section (Summary + Levels 1-10 grid) links to `/object-detect`, a public copy of the `/hp` object detection project pages that uses this same sidebar (see `CLAUDE.md`; the two copies are maintained separately). The top bar holds only the Download Resume button. Below 760px the sidebar becomes a horizontal link strip. Every new main-site page must copy this same sidebar. `tracker.html` keeps its own nav in the dark design system, with its own "Tracker" link (active), and is no longer expected to match the other two visually. If you edit the nav on `index.html` or `projects.html`, update the other of those two to match exactly.
