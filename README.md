...
| `/hp/30-60-90` | `hp/30-60-90/index.html` | Added 2026-08-27, expanded same day per review feedback. A 30/60/90-day plan for the real HP Principal TPMM, AI Solutions role, for post-interview follow-up, written in a principal-level voice (confident, but explicit about what Avi doesn't have hands-on experience with yet). First page in a "Post Interview" grouped sidebar section, visually separated from the six flat links above it. Opens with a visual Day 1/30/60/90 timeline, then Guiding Principles, a "Day One: Build While I Learn" section proposing three oil-and-gas hypothesis prototypes that extend the real `/hp/p66-example` work, the original three-phase 30/60/90 breakdown, and new sections on academia as a channel, the NVIDIA/OEM/partner ecosystem, a scraper/agent system for continuous positioning (reusing the `/hp/case-study` pattern), a proposed metrics dashboard, and a roadmap-planning cadence. No compensation figures anywhere — see `CLAUDE.md` Conventions. |
