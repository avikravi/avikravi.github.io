...
| `/hp/object-detect-summary` | `hp/object-detect-summary/index.html` | Added 2026-09-20 for 6 levels, expanded 2026-09-21 to 10. First page in a new "Object Detect" grouped sidebar section, for a separate object-detection project. Holds a 10-row "Level Roadmap" table (level, approach, status) linking to each level page below — Levels 7-10 show `TBD`/`TBD` until Avi defines them — plus a placeholder for the slide deck once Avi provides it. |
| `/hp/object-detect-level-1` through `/hp/object-detect-level-6` | `hp/object-detect-level-{1..6}/index.html` | Level 1 (Haar Cascade) and Level 3 (YOLOv8, highway traffic) added 2026-09-20 as placeholders, then written up along with Levels 2, 4, 5, 6 the same day: Level 2 (HOG+SVM, optional narrative bridge), Level 4 (YOLOv8 on a manufacturing/robotics scene — the level that actually answers the assignment brief, since Level 3's highway traffic is a smart-city example), Level 5 (video, frame-by-frame), Level 6 (tracking + unique counting across frames). Each page has an "Approach" writeup plus a dashed "Still to Fill In" checklist for what Avi hasn't added yet (demo media, code links, results). |
| `/hp/object-detect-level-7` through `/hp/object-detect-level-10` | `hp/object-detect-level-{7..10}/index.html` | Added 2026-09-21 when Avi said the assignment would have 10 levels, not 6. Pure "Content Coming Soon" placeholders (the same simple pattern Level 1 started from before it was written up) — no approach or content defined yet. |
