...
| `/hp/object-detect-level-1` through `/hp/object-detect-level-10` | `hp/object-detect-level-{1..10}/index.html` | All ten fully written up 2026-09-21 from Avi's real project — classical CV (Haar Cascade, HOG+SVM) &rarr; deep learning (YOLOv8 on general and then manufacturing scenes) &rarr; open-vocabulary detection (YOLO-World, Grounding DINO) &rarr; video (tracking/counting, speed estimation + CSV export) &rarr; forward-looking writeups on scaling to the HP ZGX Nano and Fury AI Stations. Most levels include real annotated result images and code snippets pulled from Avi's actual project; Level 8 includes real downloadable CSV data. See `CLAUDE.md` for a flagged discrepancy on Level 2's demo image and which levels still need media Avi hasn't sent yet (Level 3's aerial photo, Level 7/8's videos — too large to publish as-is). |
