...
| `games/` | **AI Arcade** (2026-09-25): a standalone, full-screen arcade "Select Your Game" page (`index.html` + `style.css` + `script.js`), opened in a new tab from the sidebar, no site sidebar. First game: Thermostat Panic (`games/thermostat/`, arcade-skinned), keep the room at 68-72°F for 60 seconds. See `CLAUDE.md` for how to add a game. |
| `object-detect/` | Public copy (2026-09-25) of the `/hp` object detection project: `/object-detect` (summary) and `/object-detect/level-1` through `-10`. Same content, but with the main-site sidebar, no interview framing, and no HP analytics grouping. Maintained separately from `/hp`. |
...
As of 2026-08-21, `index.html` and `projects.html` use a light-editorial design system (matching `hp/case-study/index.html` — see `CLAUDE.md` for the token reference), while `tracker.html` remains on the original neon dark design (its interactive UI would need separate rework to restyle safely). As of 2026-09-25, `index.html`, `projects.html`, and the `object-detect/` pages share a byte-for-byte identical, always-visible left sidebar (only the active link differs): Home, AI Arcade (opens `/games` in a new tab), then a separate Projects section (links to `projects.html`) with Robotic Surgery, Object Detect (plus its Levels 1-10 grid), and Clutch Sports Data indented under it. The top bar holds only the Download Resume button. Below 760px the sidebar becomes a horizontal link strip. Every new main-site page must copy this same sidebar. `tracker.html` keeps its own nav in the dark design system, with its own "Tracker" link (active), and is no longer expected to match the other two visually. If you edit the nav on `index.html` or `projects.html`, update the other of those two to match exactly.
...
`.claude/launch.json` wires this same command up for Claude Code's browser-preview tool (added 2026-09-21) — relative-path assets like the object-detect level pages' images only resolve correctly when served over HTTP, not opened directly as a `file://` URL.
...
| `/hp/30-60-90` | `hp/30-60-90/index.html` | Added 2026-08-27, then substantially cut down on 2026-09-16 to a terse, bullet-first, active-voice format for its real audience (a PMM Director). First page in a "Post Interview" grouped sidebar section. Opens with a Day 1/30/60/90 timeline, then an "Open Questions I Confirm First" section with literal blanks for what Avi still needs to confirm with directors/managers/teams, three oil-and-gas hypothesis cards extending the real `/hp/p66-example` work, a flat three-phase 30/60/90 breakdown (bullets + one target line per phase), a proposed metrics dashboard, and a compact "Also Worth Exploring" list (academia, OEM ecosystem, positioning-as-a-system). No compensation figures anywhere — see `CLAUDE.md` Conventions. |
| `/hp/object-detect-summary` | `hp/object-detect-summary/index.html` | Added 2026-09-20, rewritten 2026-09-21 with Avi's real project content — a take-home deliverable for an HP Principal TPMM, AI Solutions interview (round 4). Holds a Python-environment note (Intel Mac, `torch==2.2.2` ceiling), a 10-row "Level Roadmap" table linking every level page (all `Done`), and an "Overall Narrative Arc" panel describing the L1&rarr;L10 technical progression and the honesty-over-highlight-reel theme running through the project. |
| `/hp/object-detect-level-1` through `/hp/object-detect-level-10` | `hp/object-detect-level-{1..10}/index.html` | All ten fully written up 2026-09-21 from Avi's real project — classical CV (Haar Cascade, HOG+SVM) &rarr; deep learning (YOLOv8 on general and then manufacturing scenes) &rarr; open-vocabulary detection (YOLO-World, Grounding DINO) &rarr; video (tracking/counting, speed estimation + CSV export) &rarr; forward-looking writeups on scaling to the HP ZGX Nano and Fury AI Stations. Most levels show a real "Before" (unannotated original) / "After" (detection result) image pair, plus code snippets, pulled from Avi's actual project; Level 8 includes real downloadable CSV data. Level 7 embeds all 5 of Avi's real tracked clips from YouTube, in order (3 landscape uploads + 2 Shorts); Level 8's videos aren't embedded yet, still waiting on those links. See `CLAUDE.md` for a flagged discrepancy on Level 2's demo image and other still-missing media (Level 3's aerial photo). |
