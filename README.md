...
| `/hp/youtube` | `hp/youtube/index.html` | Youtube playlist of Avi's AI/ML project videos — a vertical "All Videos" list on the left (click an item to switch) and a big "now playing" player filling the rest of the row on the right. The video list (`hp/youtube/videos.json`) is manually maintained, not live-synced — see `CLAUDE.md` for why. As of 2026-08-25, a bordered "Featured Example" card sits above the playlist with a hardcoded Loom embed for "P66 Vessel Integrity Intelligence," so it's the first thing a visitor sees — see `CLAUDE.md` for how to change it. |
